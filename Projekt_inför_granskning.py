...
def sortera_atom_nummer(atom_lista):
    ...
    atom_lista.sort() #Sorterar listan baserat på atomnummer mha lt operatorn.
    ...
    while True:
        ...
        for i in range(3):
            ...
            while True:  # Loop för att kontrollera rätt inmatning
                anvandar_svar_1 = input("Vad är atomnummret för " + atom_lista[random_nummer].namn + "? Skriv 'q' för att gå tillbaka till menyn.\n")
                
                if anvandar_svar_1.lower() == 'q':
                    return
                
                if not anvandar_svar_1.isdigit():  # Kontrollera om inmatningen är ett heltal
                    print("Du kan bara mata in siffror, inget annat!")
                else:
                    break  # Avbryt while-loopen om inmatningen är giltig.
            
            if int(anvandar_svar_1) == atom_lista[random_nummer].atom_nummer:
                print("Rätt svar!\n")
                break
...
def träna_atom_beteckningar(atom_lista):
    ...
    while True:
        ...
        for i in range(3):
            ...
            while True:
                anvandar_svar_2 = input("Vad har " + str(atom_lista[random_beteckning].namn) + " för symbol" "? Skriv q för att gå tillbaka till menyn\n")
            ...
            if anvandar_svar_2 == atom_lista[random_beteckning].symbol:
                print("Rätt svar!\n")
                break
            else:
                print("Fel svar! Försök igen\n")
...
def träna_atom_namn(atom_lista):
    ...
    while True: 
        random_namn = random.randint(0, len(atom_lista) - 1)
        
        for i in range(3):

            while True:
                anvandar_svar_3 = input("Vad har atomnummer " + str(atom_lista[random_namn].atom_nummer) + " för namn" "? Skriv q för att gå tillbaka till menyn\n")

                if anvandar_svar_3.lower() == 'q':
                    return
                
                if not anvandar_svar_3.isalpha():
                    print("Du kan bara mata in bokstäver, inget annat!")
                else:
                    break

            if str(anvandar_svar_3.lower()) == atom_lista[random_namn].namn.lower():
                print("Rätt svar!\n")
                break
            else:
                print("Fel svar! Försök igen\n")
                ...
                if i == 2:
                    print("Rätta svaret va: " + str(atom_lista[random_namn].namn))
